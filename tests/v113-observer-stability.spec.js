...
const portableBundle = {
  kind: 'gringotts-import-profile-bundle', version: 1,
  generator: 'Synthetic observer fixture', exportedAt: '2026-07-10T12:00:00.000Z', profileCount: 1,
  profiles: [{
    name: 'Observer Wallet', format: 'delimited', schemaId: 'generic-signed',
    schemaLabel: 'Generic signed-amount CSV', delimiter: ',', headerSignature: 'fnv1a-abcdef12', headerCount: 5,
...
test('settles portability, import, restore, profiles, and field validation without a recursive mutation loop', async ({ app }, testInfo) => {
  ...
  await page.locator('#profileBundleFile').setInputFiles({
    name: 'observer-profile-bundle.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(portableBundle))
  });
  ...
  await page.locator('#bankImportFile').setInputFiles({
    name: 'observer-profile.csv', mimeType: 'text/csv',
