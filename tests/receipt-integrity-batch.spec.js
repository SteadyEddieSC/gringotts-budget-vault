...
async function seedTimeline(page) {
  await page.evaluate(() => {
    const receipts = [
      { importId: 'batch-receipt-3', timestamp: '2026-07-03T12:00:00.000Z', sourceFilename: 'SECRET-third.csv', sourceFingerprint: 'SECRET-repeat-fingerprint', source: 'Synthetic ledger', format: 'delimited', detectedSchema: 'Synthetic ledger', schemaConfidence: 'high', encoding: 'UTF-8', mappingSummary: 'date:Date; description:SECRET MERCHANT; amount:Amount', signMode: 'bank', dateOrder: 'mdy', warningCount: 0, transactionCount: 4, earliestDate: '2026-07-01', latestDate: '2026-07-31', exactDuplicates: 4, fuzzyCandidates: 0, insertedCount: 0, skippedCount: 4, selectedDestinationVault: 'gringottsBudgetVault.latest', destinationBeforeCount: 18, destinationAfterCount: 18, verificationResult: 'verified-no-change' },
      { importId: 'batch-receipt-2', timestamp: '2026-07-02T12:00:00.000Z', sourceFilename: 'SECRET-second.csv', sourceFingerprint: 'SECRET-repeat-fingerprint', source: 'Synthetic card activity', format: 'delimited', detectedSchema: 'Card activity export', schemaConfidence: 'high', encoding: 'UTF-8', mappingSummary: 'date:Date; description:SECRET MERCHANT; amount:Amount', signMode: 'bank', dateOrder: 'mdy', warningCount: 1, transactionCount: 4, earliestDate: '2026-06-01', latestDate: '2026-06-30', exactDuplicates: 1, fuzzyCandidates: 1, insertedCount: 2, skippedCount: 2, selectedDestinationVault: 'gringottsBudgetVault.latest', destinationBeforeCount: 16, destinationAfterCount: 18, verificationResult: 'verified' },
      { importId: 'batch-receipt-1', timestamp: '2026-07-01T12:00:00.000Z', sourceFilename: 'SECRET-first.csv', sourceFingerprint: 'SECRET-first-fingerprint', source: 'Synthetic signed ledger', format: 'delimited', detectedSchema: 'Generic signed-amount CSV', schemaConfidence: 'high', encoding: 'UTF-8', mappingSummary: 'date:Date; description:SECRET MERCHANT; amount:Amount', signMode: 'bank', dateOrder: 'mdy', warningCount: 0, transactionCount: 6, earliestDate: '2026-05-01', latestDate: '2026-05-31', exactDuplicates: 2, fuzzyCandidates: 0, insertedCount: 4, skippedCount: 2, selectedDestinationVault: 'gringottsBudgetVault.latest', destinationBeforeCount: 12, destinationAfterCount: 16, verificationResult: 'verified' }
    ];
    ...
    localStorage.setItem('gringottsImportBatchIndex.v1', JSON.stringify({
      version: 1, updatedAt: '2026-07-02T12:00:01.000Z', links: [{
        ...
        normalizedRowCount: 4, wouldInsert: 2, wouldSkip: 2, validationErrorCount: 0,
        validationWarningCount: 1, transactionWriteReady: true, verifiedCounts: true,
        dataBoundary: { transactionRowsIncluded: false, sourceFileNameIncluded: false, sourceFingerprintIncluded: false, destinationStorageKeyIncluded: false, accountIdentifiersIncluded: false, merchantNamesIncluded: false, vaultContentsIncluded: false }
      }]
    }));
  });
}
...
test('links an explicit current dry run to the verified receipt without changing writer safeguards', async ({ app }, testInfo) => {
  ...
  const [backup] = await Promise.all([page.waitForEvent('download'), page.locator('#prepareBankImportBackup').click()]);
  ...
  const result = await page.evaluate(() => ({ vaultCount: JSON.parse(localStorage.getItem('gringottsBudgetVault.latest')).transactions.length, index: JSON.parse(localStorage.getItem('gringottsImportBatchIndex.v1')), receipt: JSON.parse(localStorage.getItem('gringottsImportHistory.v1')).imports[0] }));
...
test('downloads sanitized full and selected timeline packages', async ({ app }) => {
  ...
  await seedTimeline(page);
  await openImport(page);
  const [fullDownload] = await Promise.all([page.waitForEvent('download'), page.locator('#downloadReceiptTimeline').click()]);
  const fullPayload = JSON.parse(await fs.readFile(await fullDownload.path(), 'utf8'));
  await page.locator('[data-v121-batch-select]').first().click();
  const [selectedDownload] = await Promise.all([page.waitForEvent('download'), page.locator('#downloadSelectedReceiptBatch').click()]);
