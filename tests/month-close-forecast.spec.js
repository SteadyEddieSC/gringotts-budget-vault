...
async function cleanSelectedMonth(page) {
  await page.evaluate(async () => {
    ...
    vault.transactions = vault.transactions.map((transaction) => {
      ...
      return { ...transaction, pending: false, reviewed: true, category: String(transaction.category || '').toLowerCase() === 'other' ? 'Household' : transaction.category };
    });
...
test('preserves close, forecast, debt, scenario, and trend features under the v127 presentation shell', async ({ app }) => {
  const { page } = app;
...
test('reconciles, closes, and reopens without changing transactions', async ({ app }, testInfo) => {
  ...
  const closed = await page.evaluate(() => ({ vaultCount: JSON.parse(localStorage.getItem('gringottsBudgetVault.latest')).transactions.length, close: JSON.parse(localStorage.getItem('gringottsMonthClose.v1')) }));
  ...
  const reopened = await page.evaluate(() => ({ vaultCount: JSON.parse(localStorage.getItem('gringottsBudgetVault.latest')).transactions.length, events: JSON.parse(localStorage.getItem('gringottsMonthClose.v1')).months['2026-07'].events }));
...
test('adds a promotional APR debt and records a payment without touching the vault', async ({ app }, testInfo) => {
  ...
  const result = await page.evaluate(() => ({ vaultCount: JSON.parse(localStorage.getItem('gringottsBudgetVault.latest')).transactions.length, debt: JSON.parse(localStorage.getItem('gringottsDebtPlan.v1')).debts[0] }));
