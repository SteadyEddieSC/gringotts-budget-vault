...
test('downloads the v110 24-sheet Vault Workbook and exposes the annual tracker input', async ({ app }, testInfo) => {
  ...
  await expect(page.getByText(/24-sheet workbook/i)).toBeVisible();

  const [download] = await Promise.all([
    page.waitForEvent('download'),
    page.locator('#vaultXlsx').click()
  ]);
  expect(download.suggestedFilename()).toMatch(/Gringotts_Budget_Vault_v110_2026-07_.*\.xlsx/i);
});
...
test('downloads a full backup from Tools instead of the header', async ({ app }, testInfo) => {
  ...
  expect(download.suggestedFilename()).toMatch(/Gringotts_v110_backup_12_/i);
});
