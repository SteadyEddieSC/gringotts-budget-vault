...
const noRemote = (source, label) => expect(source, `${label} must remain local-first`).not.toMatch(/\bfetch\s*\(|XMLHttpRequest|sendBeacon|WebSocket/);
...
test('parser preflight checks inherited and current release modules plus strict TypeScript before browser installation', () => {
  ...
  for (const excluded of ['src/v127/release.js', 'src/v128/contracts.ts', 'src/v129/workflow-evidence.ts']) expect(workflow).not.toContain(`node --check ${excluded}`);
  expectAll(packageJson, ['"typecheck": "tsc -p tsconfig.json"', '"typescript": "5.9.2"']);
...
test('analytical, planning, portability, and workflow-evidence layers remain local and do not silently write the vault', () => {
  ...
  for (const file of [
    ...
    'src/v128/portable-vault.js', 'src/boot-v128.js', 'src/v129/workflow-evidence.js', 'src/v129/workflow-review.js', 'src/boot-v129.js'
...
test('bounded metadata stores remain separate from transactions', () => {
  ...
  for (const file of ['src/boot-v127.js', 'src/boot-v128.js', 'src/v129/workflow-evidence.js', 'src/v129/workflow-review.js', 'src/boot-v129.js']) {
...
test('v127 retains the budget-neutral presentation policy without runtime or storage ownership', () => {
  ...
  expectAll(boot, ["import './boot-v126.js?v=127base2'", 'observerAdded: false', 'storageWritesAdded: false', 'networkBudgetDelta: 0', 'Retained v113 household-insight styles are consolidated here']);
...
test('v128 adds strict portable-vault contracts without network, storage, encryption, or cloud implementation', () => {
  ...
  expectAll(boot, ["import './boot-v126.js?v=128base1'", 'typeScriptStrict: true', 'encryptionReady: false', 'cloudAdaptersEnabled: false', 'networkImplementationAdded: false', 'networkBudgetDelta: 0']);
  expect(boot).not.toContain('new MutationObserver');
  expect(boot).not.toMatch(/localStorage|sessionStorage/);
});
...
test('v129 adds manual workflow evidence without telemetry, persistence, financial reads, or automatic decisions', () => {
  ...
  expectAll(controller, ['const reviewState = new Map()', 'Download Local Review JSON']);
  expect(controller).not.toMatch(/\bfetch\s*\(|XMLHttpRequest|sendBeacon|WebSocket|localStorage|sessionStorage|indexedDB/);
  expectAll(boot, [
    "import './boot-v128.js?v=129base2'", "import('./v129/workflow-review.js?v=129evidence2')",
    "window.addEventListener('click',handleClick,true)", 'automaticTelemetry: false', 'financialDataRead: false',
    'persistentStoreAdded: false', 'networkImplementationAdded: false', 'lazyController: true', 'networkBudgetDelta: 0'
  ]);
  expect(boot).not.toContain('new MutationObserver');
  expect(boot).not.toMatch(/localStorage|sessionStorage/);
});
...
test('v129 owns the live static shells while preserving startup budgets through lazy loading', () => {
  ...
  expectAll(index, ['src/boot-v129.js?v=129evidence1', 'styles/v106-v107.css', 'styles/v116.css']);
  expectAll(app, ['src/boot-v129.js?v=129evidence1', 'styles/v106-v107.css', 'styles/v116.css']);
  for (const source of [index, app]) {
    for (const absent of [
      'styles/v106.css', 'styles/v107.css', 'styles/v113.css', 'styles/v127.css', 'styles/v128.css', 'styles/v129.css',
...
test('public repository quality and release-control files remain present through v129', () => {
  const required = [
    ...
    'src/v129/workflow-evidence.ts', 'src/v129/workflow-evidence.js', 'src/v129/workflow-review.js', 'tsconfig.json',
    ...
    'tests/v128-portable-vault-foundation.spec.js', 'tests/v129-workflow-evidence-review.spec.js', 'BANK_IMPORT_ROADMAP.md',
