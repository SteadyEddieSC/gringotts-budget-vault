...
test('feeds insights into v117 reports, the 33-sheet workbook, and the meeting pack', async ({ app }, testInfo) => {
  ...
  expect(workbook.suggestedFilename()).toMatch(/Gringotts_Budget_Vault_v117_2026-07-01_to_2026-07-31_.*\.xlsx/i);

  const [meeting] = await Promise.all([page.waitForEvent('download'), page.locator('#meetingMd').click()]);
  expect(meeting.suggestedFilename()).toMatch(/Gringotts_Family_Meeting_Pack_v117_2026-07-01_to_2026-07-31_.*\.md/i);
});
