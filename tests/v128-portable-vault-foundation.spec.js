...
test('round-trips a portable vault in the browser without network or storage mutation', async ({ app }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    try {
      ...
      const packageValue = await module.createPortableVaultPackage(vault, {
      ...
      return {
        before,
        after: Object.fromEntries(Object.entries(localStorage)),
        requests,
        valid: parsed.valid,
        count: parsed.package.manifest.transactionCount,
        digest: parsed.package.manifest.integrity.digest,
        privacy: parsed.package.manifest.privacy,
        filename: module.portableVaultFilename('2026-08-03T03:30:00.000Z')
      };
    } finally {
...
test('rejects portable-vault tampering before any restore decision', async ({ app }) => {
  ...
  const message = await page.evaluate(async () => {
    ...
    const vault = JSON.parse(localStorage.getItem('gringottsBudgetVault.latest'));
    const packageValue = await module.createPortableVaultPackage(vault, {
      createdAt: '2026-08-03T03:30:00.000Z', sourceRelease: 'v128'
    });
    packageValue.vault.transactions[0].amount = Number(packageValue.vault.transactions[0].amount || 0) + 1;
    try {
      await module.validatePortableVaultPackage(packageValue);
      return '';
    } catch (error) {
