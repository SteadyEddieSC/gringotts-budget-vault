...
test('boots v130 and navigates the complete household report preview', async ({ app }) => {
  const { page } = app;
...
test('downloads the 43-sheet workbook, guided plan, and range CSV', async ({ app }, testInfo) => {
  ...
  expect(plan.suggestedFilename()).toMatch(/Gringotts_Guided_Household_Plan_v126_2026-07_.*\.md/i);
