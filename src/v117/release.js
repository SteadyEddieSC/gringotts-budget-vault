...
let profileEnhancerPromise = null;
...
function requiredFeature(name) {
  ...
  if (typeof value !== 'function') throw new Error(`v117 feature dependency is not ready: ${name}`);
  return value;
}
...
function downloadBackupV117() {
  const candidate = best();
  if (!candidate?.obj || candidate.transactions < 1) {
    announce('No populated readable vault is available to back up');
    return;
  }
  downloadJson(`Gringotts_v117_backup_${candidate.transactions}_${stamp()}.json`, candidate.obj);
  announce('Current vault backup downloaded');
}

function handleV117Download(event) {
  ...
  if (button.id === 'vaultXlsx') {
    ...
    download(`Gringotts_Budget_Vault_v117_${reportSlug(model)}_${stamp()}.xlsx`, xlsxBlob(expandedWorkbookSheetsV115(getMonth(), model)));
    announce('33-sheet Vault Workbook downloaded');
    return;
  }
  if (button.id === 'meetingMd') {
    const model = householdReportModel();
    download(`Gringotts_Family_Meeting_Pack_v117_${reportSlug(model)}_${stamp()}.md`, familyMeetingMarkdown(model), 'text/markdown');
    announce('Family meeting pack downloaded');
    return;
  }
  if (button.id === 'planMd') {
    const plan = guidedPlanModel();
    download(`Gringotts_Guided_Household_Plan_v117_${plan.month}_${stamp()}.md`, guidedPlanMarkdown(plan), 'text/markdown');
  ...
  if (button.id === 'exportRules') {
    downloadJson(`Gringotts_v117_rules_review_${stamp()}.json`, reviewPackage());
    return;
  }
  if (button.id === 'exportIcs' || button.id === 'downloadIcs') {
    download(`Gringotts_v117_calendar_${stamp()}.ics`, ics(), 'text/calendar');
    return;
  }

  const runtime = window.GringottsCleanRuntime;
  downloadJson(`Gringotts_v117_diagnostics_${stamp()}.json`, {
    ...debugReport(),
...
function installV117DownloadOverrides() {
  if (downloadOverridesInstalled) return;
  downloadOverridesInstalled = true;
  document.addEventListener('click', handleV117Download, true);
}
...
function enhanceReportCenter(center) {
  if (center.dataset.v117Enhanced !== 'true') {
    center.dataset.v117Enhanced = 'true';
    ...
    center.querySelectorAll('.report-kicker').forEach((node) => setText(node, 'Import Profiles & Validation v117'));
...
function createImportProgress() {
  ...
  progress.innerHTML = `<li><strong>1. Inspect</strong><span>Choose a local export and confirm how it was detected.</span><small></small></li>
    <li><strong>2. Map</strong><span>Apply or save an exact-compatible profile, then validate dates, amounts, accounts, and identifiers.</span><small></small></li>
...
function enhanceProfiles(page) {
  if (!profileEnhancerPromise) profileEnhancerPromise = import('./import-profiles.js?v=117profiles1');
  profileEnhancerPromise
    .then((module) => module.enhanceImportProfiles(page))
    .catch((error) => announce(error?.message || 'Import profiles could not be loaded'));
}

function enhanceImportPage(page) {
  if (page.dataset.v117Enhanced !== 'true') {
    page.dataset.v117Enhanced = 'true';
    ...
    setText(titleRow?.querySelector('p'), 'Reuse exact-compatible local mapping profiles, validate every field, add reviewed missing transactions, or switch to full restore.');
    setText(titleRow?.querySelector('.section-meta'), 'Profiles store metadata only');
...
function enhanceRoadmap(root) {
  ...
  if (!section || section.dataset.v117Enhanced === 'true') return;
  section.dataset.v117Enhanced = 'true';
  ...
  setText(titleRow?.querySelector('.section-meta'), 'Next: v118');
  const shipped = section.querySelector('.roadmap-item.shipped');
  if (shipped) shipped.innerHTML = '<h3>v117 — Import Profiles & Field Validation</h3><p>Exact-compatible metadata-only profiles, explicit profile management, automatic single-match application, and field-level validation explanations without changing guarded transaction writes.</p>';
  const roadmap = section.querySelector('.roadmap');
  if (roadmap) roadmap.innerHTML = '<article class="roadmap-item"><h3>v118 — Profile Portability & Institution Patterns</h3><p>Export and import sanitized profile definitions with conflict review, add more fictional institution patterns, and improve compatibility explanations without storing source rows.</p></article><article class="roadmap-item"><h3>Future household planning</h3><p>Scenario comparisons, recurring-cost decisions, account cleanup, and close-history analysis remain candidates only after actual household use shows clear value.</p></article>';
}
...
export function prepareV117Interceptors() {
  installV117DownloadOverrides();
}

export function activateV117() {
  Object.assign(BUILD, {
    version: 'v117',
    name: 'Import Profiles & Field Validation',
    runtime: 'src/runtime-v111-reporting.js + lazy src/v115 + src/v117',
    cacheBust: '117profiles1'
  });
  ...
  const registry = window.GringottsV117 || (window.GringottsV117 = {});
