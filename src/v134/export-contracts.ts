export const EXPORT_CONTRACT_RELEASE = 'v134' as const;
export const WORKBOOK_SHEET_CAP = 43 as const;

export type ExportFormat = 'json' | 'markdown' | 'xlsx' | 'ics';
export type ExportPrivacyMode =
  | 'full-vault'
  | 'transaction-detail'
  | 'household-summary'
  | 'configuration-only'
...
export interface ExportContract {
  ...
  failureBehavior: 'throw-without-substitution';
  cancellationBehavior: 'abort-before-dispatch';
  retryBehavior: 'none';
}

export interface WorkbookOwnershipGroup {
  id: string;
  owner: string;
  count: number;
...
] as const satisfies readonly ExportContract[];

export type ExportId = typeof CONTRACTS[number]['id'];
export const EXPORT_CATALOG: readonly ExportContract[] = Object.freeze(CONTRACTS.map((entry) => Object.freeze({ ...entry })));

export const WORKBOOK_OWNERSHIP: readonly WorkbookOwnershipGroup[] = Object.freeze([
...
function fail(message: string): never {
  throw new Error(`Export contract rejected: ${message}`);
}
...
function timestamp(value = new Date().toISOString()): string {
  return exactIso(value).replace(/[:.]/g, '-');
}
...
export function getExportContract(id: ExportId): ExportContract {
  const contract = EXPORT_CATALOG.find((entry) => entry.id === id);
  if (!contract) fail(`unknown export id ${id}`);
  return contract;
}

export function buildExportFilename(id: ExportId, context: ExportFilenameContext = {}): string {
  ...
  const names: Record<ExportId, string> = {
...
function forbiddenKeys(mode: ExportPrivacyMode): ReadonlySet<string> | null {
...
function inspect(value: unknown, forbidden: ReadonlySet<string>, path = 'payload', seen = new WeakSet<object>()): void {
...
export function assertExportPayloadSafe(id: ExportId, payload: unknown): true {
  const forbidden = forbiddenKeys(getExportContract(id).privacyMode);
  if (forbidden) inspect(payload, forbidden);
  return true;
}

export function validateExportCatalog(): true {
...
export function validateWorkbookOwnership(): true {
