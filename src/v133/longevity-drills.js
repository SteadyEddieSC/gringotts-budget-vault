...
export const LONGEVITY_DRILL_VERSION = 1;
export const LONGEVITY_DRILL_KIND = 'gringotts-local-data-longevity-drill';
...
function report(scenario, disposition, summary, evidence, humanAction) {
  return {
...
function runUpgrade(input) {
  ...
  const after = canonicalJson(JSON.parse(before));
...
async function runCorruption(input) {
...
function runRollback(input) {
  ...
  const restored = JSON.parse(backupBefore);
...
function runOrphan(input) {
  ...
  const disposition = orphanIds.length ? 'manual-review' : 'pass';
...
export async function runLongevityDrill(scenario, input) {
...
export function createSyntheticLongLivedVault(transactionCount = 144) {
