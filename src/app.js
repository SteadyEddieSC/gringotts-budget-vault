(function(){
  ...
  function bind(){
    ...
    GBV.ui.$('#clearDataBtn').onclick=()=>{if(confirm('Clear local Gringotts data on this device? Download a backup first if needed.')){GBV.store.clear(); GBV.ui.renderAll(); GBV.ui.renderImportSummary('Local vault cleared.');}};
    ...
    GBV.ui.$('#sampleBtn').onclick=()=>{const r=GBV.importer.importPack(GBV.importer.demo(),'demo-data-v59.json'); GBV.ui.renderImportSummary(`Loaded demo data. Added ${r.added}, total ${r.total}.`); GBV.ui.renderAll();};
  ...
  document.addEventListener('DOMContentLoaded',()=>{GBV.store.load(); bind(); GBV.ui.renderAll(); GBV.pwa.installPrompt(); GBV.pwa.register();});
})();
