...
function manifestObject(value) {
  ...
  if (!Number.isSafeInteger(value.transactionCount) || value.transactionCount < 1) fail('manifest transactionCount must be a positive integer');
...
export async function createPortableVaultPackage(vaultInput, options = {}) {
...
export function serializePortableVaultPackage(value) {
  return `${canonicalJson(value)}\n`;
}

export async function validatePortableVaultPackage(value) {
...
export async function parsePortableVaultPackage(text) {
...
export function portableVaultFilename(createdAt = new Date().toISOString()) {
