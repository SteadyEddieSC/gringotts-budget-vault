...
export type PerformanceBudgetName =
  | 'routeReadyMs'
  | 'enhancementMs'
  | 'enhancementPasses'
  | 'observerCallbacks'
...
export interface PerformanceBudgetCheck {
  ...
  ok: boolean;
  unit: 'ms' | 'count' | 'bytes' | 'boolean';
}

export interface PerformanceBudgetEvaluation {
  release: 'v130';
  ok: boolean;
  checks: ReadonlyArray<PerformanceBudgetCheck>;
  failures: ReadonlyArray<string>;
  budgets: typeof V130_PERFORMANCE_BUDGETS;
}
...
function numericCheck(
  ...
  actual: number,
  limit: number,
  unit: PerformanceBudgetCheck['unit']
): PerformanceBudgetCheck {
  return Object.freeze({ name, actual: finiteNonNegative(actual, name), limit, ok: actual <= limit, unit });
}

function booleanCheck(name: PerformanceBudgetName, actual: boolean): PerformanceBudgetCheck {
  return Object.freeze({ name, actual, limit: true, ok: actual === true, unit: 'boolean' });
}

export function evaluatePerformanceBudget(input: PerformanceBudgetInput): PerformanceBudgetEvaluation {
