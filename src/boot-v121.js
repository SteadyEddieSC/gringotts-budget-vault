...
function renderFailure(error) {
  ...
  boot.innerHTML = `
    <section class="boot-card" role="alert">
      ...
      <p>The page shell loaded, but a JavaScript module failed. Your browser-local vault, import profiles, revision history, profile bundles, dry-run metadata, receipt audits, batch index, import receipts, Guided Plan, insights, goals, close history, report range, forecast settings, and debt plan have not been cleared or overwritten.</p>
      <div class="boot-actions">
        <button id="bootRetry" type="button">Retry v121</button>
...
function loadRouteLayers() {
  if (!routeLayersPromise) {
    routeLayersPromise = Promise.all([
      import('./v118/release.js?v=121batch1'),
      import('./v119/release.js?v=121batch1'),
      import('./v120/release.js?v=121batch1'),
      import('./v121/release.js?v=121batch1')
    ]).then(async ([v118, v119, v120, v121]) => {
...
import('./runtime-v111-reporting.js?v=121batch1')
  .then(async () => {
    const [{ activateV115 }, { installAccessibilityEnhancements }] = await Promise.all([
      import('./v115/release.js?v=121batch1'),
      import('./v112/accessibility.js?v=121batch1')
    ]);
    const build = activateV115();
    Object.assign(build, {
      version: 'v121',
      name: 'Receipt Integrity & Import Batch Reconciliation',
      runtime: 'src/runtime-v111-reporting.js + lazy src/v115 + lazy v118 portability + lazy v119 diagnostics + lazy v120 audit + lazy v121 receipt lineage',
      cacheBust: '121batch1'
    });
    ...
    Object.assign(registry, { release: 'v121', loadRouteLayers });
