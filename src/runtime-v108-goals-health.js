...
import {
  addGoalContribution, archiveGoal, deleteGoal, exportExpandedVaultWorkbookV108,
  saveGoal, saveHealthSnapshot
} from './v108/goals.js';

Object.assign(BUILD, {
  version: 'v108',
  name: 'Goals & Vault Health',
  runtime: 'src/runtime-v108-goals-health.js',
  cacheBust: '108goalshealth1'
});
...
function downloadBackup() {
  ...
  downloadJson(`Gringotts_v108_backup_${candidate.transactions}_${stamp()}.json`, candidate.obj);
...
function bindCalendar() {
  ...
  $('downloadIcs')?.addEventListener('click', () => download(`Gringotts_v108_calendar_${stamp()}.ics`, ics(), 'text/calendar'));
...
function bindExistingActions() {
  ...
  $('exportReview')?.addEventListener('click', () => downloadJson(`Gringotts_v108_rules_review_${stamp()}.json`, reviewPackage()));
  ...
  $('vaultXlsx')?.addEventListener('click', () => exportExpandedVaultWorkbookV108(toast));
  ...
  $('exportRules')?.addEventListener('click', () => downloadJson(`Gringotts_v108_rules_review_${stamp()}.json`, reviewPackage()));
  $('exportIcs')?.addEventListener('click', () => download(`Gringotts_v108_calendar_${stamp()}.ics`, ics(), 'text/calendar'));
  $('exportDebug')?.addEventListener('click', () => downloadJson(`Gringotts_v108_diagnostics_${stamp()}.json`, { ...debugReport(), performance: { lastRenderMs } }));
  ...
  $('downloadDebug')?.addEventListener('click', () => downloadJson(`Gringotts_v108_diagnostics_${stamp()}.json`, { ...debugReport(), performance: { lastRenderMs } }));
}
