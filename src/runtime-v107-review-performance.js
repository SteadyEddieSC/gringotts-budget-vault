...
Object.assign(BUILD, {
  ...
  runtime: 'src/runtime-v107-review-performance.js',
  cacheBust: '107reviewperf1'
});
...
function downloadBackup() {
  ...
  downloadJson(`Gringotts_v107_backup_${candidate.transactions}_${stamp()}.json`, candidate.obj);
...
function bindSubnavigation() {
  document.querySelectorAll('[data-money-section]').forEach((button) => button.addEventListener('click', () => { moneySection = button.dataset.moneySection; scheduleRender(); }));
...
function bindCalendar() {
  ...
  $('downloadIcs')?.addEventListener('click', () => download(`Gringotts_v107_calendar_${stamp()}.ics`, ics(), 'text/calendar'));
...
function bindReview() {
  $('enableReviewEditing')?.addEventListener('click', () => {
    if (beginReviewSession(toast)) scheduleRender();
  });
  ...
  $('useReviewSuggestion')?.addEventListener('click', (event) => {
    const input = $('reviewCategory');
    if (input) {
      input.value = event.currentTarget.dataset.suggestion || '';
      input.focus();
    }
  });
  $('saveReviewProgress')?.addEventListener('click', (event) => {
    if (saveTransactionReview(event.currentTarget.dataset.rowIndex, reviewChanges(), false, toast)) scheduleRender();
  });
  $('saveReviewAndNext')?.addEventListener('click', (event) => {
    if (saveTransactionReview(event.currentTarget.dataset.rowIndex, reviewChanges(), true, toast)) scheduleRender();
  });
  $('batchReviewCategorized')?.addEventListener('click', () => {
    const confirmed = window.confirm('Mark every queued transaction in the selected month that already has a specific category as reviewed?\n\nThis does not change categories, amounts, merchants, or dates.');
    if (!confirmed) return;
    if (batchMarkCategorizedReviewed(getMonth(), toast)) {
      reviewPosition = 0;
      scheduleRender();
    }
  });
...
function bindExistingActions() {
  ...
  $('exportReview')?.addEventListener('click', () => downloadJson(`Gringotts_v107_rules_review_${stamp()}.json`, reviewPackage()));
  ...
  $('exportRules')?.addEventListener('click', () => downloadJson(`Gringotts_v107_rules_review_${stamp()}.json`, reviewPackage()));
  $('exportIcs')?.addEventListener('click', () => download(`Gringotts_v107_calendar_${stamp()}.ics`, ics(), 'text/calendar'));
  $('exportDebug')?.addEventListener('click', () => downloadJson(`Gringotts_v107_diagnostics_${stamp()}.json`, { ...debugReport(), performance: { lastRenderMs } }));
  ...
  $('downloadDebug')?.addEventListener('click', () => downloadJson(`Gringotts_v107_diagnostics_${stamp()}.json`, { ...debugReport(), performance: { lastRenderMs } }));
}
