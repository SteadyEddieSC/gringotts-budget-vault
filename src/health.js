(function(){
  ...
  function bestVaultCount(){try{const best=GBV.store&&GBV.store.bestVault?GBV.store.bestVault():null; return best?best.tx:null;}catch(e){return null;}}
  function run(){
  ...
  function render(){
    ...
    if(summary){summary.textContent=summaryText(out); summary.dataset.healthBase='1';}
    ...
    if(window.GringottsReleaseSync&&window.GringottsReleaseSync.renderStatus) window.GringottsReleaseSync.renderStatus();
  }
