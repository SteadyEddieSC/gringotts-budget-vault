...
function renderFailure(error) {
  ...
  boot.innerHTML = `
    <section class="boot-card" role="alert">
      ...
      <p>The page shell loaded, but a JavaScript module failed. Your browser-local vault, import profiles, import receipts, Guided Plan, insights, goals, close history, report range, forecast settings, and debt plan have not been cleared or overwritten.</p>
      <div class="boot-actions">
        <button id="bootRetry" type="button">Retry v117</button>
...
import('./runtime-v111-reporting.js?v=117profiles1')
  .then(async () => {
    const [{ activateV115 }, { prepareV117Interceptors, activateV117 }, { installAccessibilityEnhancements }] = await Promise.all([
      import('./v115/release.js?v=117profiles1'),
      import('./v117/release.js?v=117profiles1'),
      import('./v112/accessibility.js?v=117profiles1')
    ]);
    prepareV117Interceptors();
    activateV115();
    const build = activateV117();
