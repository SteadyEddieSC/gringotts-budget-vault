...
function renderFailure(error) {
  ...
  boot.innerHTML = `
    <section class="boot-card" role="alert">
      ...
      <p>The page shell loaded, but a JavaScript module failed. Your browser-local vault, import profiles, profile bundles, import receipts, Guided Plan, insights, goals, close history, report range, forecast settings, and debt plan have not been cleared or overwritten.</p>
      <div class="boot-actions">
        <button id="bootRetry" type="button">Retry v118</button>
...
import('./runtime-v111-reporting.js?v=118portable1')
  .then(async () => {
    const [{ activateV115 }, { prepareV118Interceptors, activateV118 }, { installAccessibilityEnhancements }] = await Promise.all([
      import('./v115/release.js?v=118portable1'),
      import('./v118/release.js?v=118portable1'),
      import('./v112/accessibility.js?v=118portable1')
    ]);
    prepareV118Interceptors();
    activateV115();
    const build = activateV118();
