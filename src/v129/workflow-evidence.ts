export const WORKFLOW_REVIEW_KIND = 'gringotts-workflow-evidence-review' as const;
export const WORKFLOW_REVIEW_VERSION = 1 as const;
export const WORKFLOW_INVENTORY_VERSION = 1 as const;
export const WORKFLOW_REVIEW_RELEASE = 'v129' as const;

export type WorkflowDestination = 'Dashboard' | 'Money' | 'Calendar' | 'Reports' | 'Activity' | 'Tools';
export type WorkflowUsage = 'unreviewed' | 'never' | 'occasional' | 'regular' | 'essential';
export type WorkflowFriction = 'unreviewed' | 'none' | 'low' | 'medium' | 'high';
export type WorkflowOutcome = 'unreviewed' | 'successful' | 'partial' | 'blocked' | 'unclear';
export type WorkflowSignal = 'none' | 'works-well' | 'repeated-confusion' | 'slow-path' | 'abandoned' | 'duplicate-surface' | 'missing-guidance' | 'failure-recovery' | 'unmet-need';
export type WorkflowDisposition = 'unreviewed' | 'keep' | 'simplify' | 'consolidate' | 'demote' | 'remove' | 'investigate';
...
export interface WorkflowDefinition {
  ...
  label: string;
  purpose: string;
  cadence: 'daily' | 'weekly' | 'monthly' | 'as-needed';
...
export interface WorkflowReviewSummary {
  ...
  highFrictionWorkflowIds: string[];
  consolidationCandidateIds: string[];
  unmetNeedWorkflowIds: string[];
  keepCandidateIds: string[];
  recommendedNextAction: string;
}

export interface WorkflowReviewBundle {
  kind: typeof WORKFLOW_REVIEW_KIND;
  version: typeof WORKFLOW_REVIEW_VERSION;
  release: typeof WORKFLOW_REVIEW_RELEASE;
  inventoryVersion: typeof WORKFLOW_INVENTORY_VERSION;
  createdAt: string;
  privacy: {
...
export const WORKFLOW_INVENTORY: readonly WorkflowDefinition[] = Object.freeze([
...
const USAGES = new Set<WorkflowUsage>(['unreviewed', 'never', 'occasional', 'regular', 'essential']);
const FRICTIONS = new Set<WorkflowFriction>(['unreviewed', 'none', 'low', 'medium', 'high']);
const OUTCOMES = new Set<WorkflowOutcome>(['unreviewed', 'successful', 'partial', 'blocked', 'unclear']);
const SIGNALS = new Set<WorkflowSignal>(['none', 'works-well', 'repeated-confusion', 'slow-path', 'abandoned', 'duplicate-surface', 'missing-guidance', 'failure-recovery', 'unmet-need']);
const DISPOSITIONS = new Set<WorkflowDisposition>(['unreviewed', 'keep', 'simplify', 'consolidate', 'demote', 'remove', 'investigate']);
...
export function emptyWorkflowObservation(workflowId: string): WorkflowObservation {
...
export function sanitizeWorkflowNote(value: unknown): string | undefined {
...
export function validateWorkflowObservation(value: WorkflowObservation): WorkflowObservation {
...
export function isWorkflowObservationReviewed(value: WorkflowObservation): boolean {
...
export function isWorkflowObservationComplete(value: WorkflowObservation): boolean {
  const item = validateWorkflowObservation(value);
  return item.usage !== 'unreviewed' && item.friction !== 'unreviewed' && item.outcome !== 'unreviewed' && item.disposition !== 'unreviewed';
}

export function summarizeWorkflowReview(values: readonly WorkflowObservation[]): WorkflowReviewSummary {
...
export function buildWorkflowReviewBundle(values: readonly WorkflowObservation[], createdAt = new Date().toISOString()): WorkflowReviewBundle {
...
export function workflowReviewSummaryText(bundle: WorkflowReviewBundle): string {
...
export function workflowReviewFilename(createdAt = new Date().toISOString()): string {
