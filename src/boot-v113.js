...
function renderFailure(error) {
  ...
  boot.innerHTML = `
    <section class="boot-card" role="alert">
      ...
      <p>The page shell loaded, but a JavaScript module failed. Your browser-local vault, insights, goals, import history, close history, report range, forecast settings, and debt plan have not been cleared or overwritten.</p>
      <div class="boot-actions">
        <button id="bootRetry" type="button">Retry v113</button>
...
import('./runtime-v111-reporting.js?v=113insights1')
  .then(async () => {
    const [{ activateV113 }, { installAccessibilityEnhancements }] = await Promise.all([
      import('./v113/release.js?v=113insights1'),
      import('./v112/accessibility.js?v=113insights1')
    ]);
    const build = activateV113();
