...
export function activityView(section = 'ledger', search = '', reviewPosition = 0) {
  let content = '';
  if (section === 'plan') content = guidedPlanningView();
  else if (section === 'insights') content = insightsView();
  else {
  ...
  return `<div class="workspace"><div class="subnav activity-subnav" role="tablist" aria-label="Activity sections"><button class="subtab ${section === 'ledger' ? 'active' : ''}" data-activity-section="ledger">Transactions</button><button class="subtab ${section === 'review' ? 'active' : ''}" data-activity-section="review">Review Queue</button><button class="subtab ${section === 'rules' ? 'active' : ''}" data-activity-section="rules">Rules</button><button class="subtab ${section === 'insights' ? 'active' : ''}" data-activity-section="insights">Insights</button><button class="subtab ${section === 'plan' ? 'active' : ''}" data-activity-section="plan">Plan</button></div>${content}</div>`;
}
...
function downloadOptions(template, model) {
  return `<div class="grid two report-downloads screen-only">
    ...
    <article class="card report-option"><h3>Guided household plan</h3><p>Download the current explainable checklist with saved owner, target, status, and notes.</p><button id="planMd" class="btn secondary">Download Guided Plan</button></article>
  </div>`;
}
...
function meetingPage(model, insights, plan) {
  ...
  const planRisks = plan.active.filter((item) => ['urgent', 'high'].includes(item.priority)).slice(0, 4).map((item) => item.why);
  const risks = [...new Set([...planRisks, ...insightRisks, ...model.meeting.risks])].slice(0, 10);
  const planActions = plan.active.slice(0, 6).map((item) => `${item.title}: ${item.nextStep}`);
  const actions = [...new Set([...planActions, ...model.meeting.actions])].slice(0, 12);
  return `<section class="printable-report report-page"><h2>Family meeting brief</h2><div class="grid two meeting-report-grid">${section('Questions to Decide Together', questions)}${section('Wins', model.meeting.wins, 'report-wins')}${section('Risks & Watch Items', risks, 'report-risks')}${section('Action Items', actions, 'report-actions')}</div></section>`;
}
